...
const Color _kCircleActiveLight = Colors.black;
...
class _CustomStepperState extends State<CustomStepper> with TickerProviderStateMixin {
  ...
  Color _circleColor(int index) {
    ...
    if (!_isDark()) {
      if (widget.steps[index].isActive) {
        return CustomColor.btnColor;
      } else if (widget.steps[index].isCurrentIndex) {
        return CustomColor.yellowColor;
      } else {
  ...
  TextStyle _labelStyle(int index) {
    ...
    switch (widget.steps[index].state) {
      case CustomStepState.indexed:
      case CustomStepState.editing:
        return const TextStyle(color: CustomColor.hintColor, fontSize: 12);
      case CustomStepState.complete:
        return const TextStyle(color: CustomColor.btnColor, fontSize: 12);
      case CustomStepState.disabled:
        return textTheme.bodyText1!.copyWith(
          color: _isDark() ? _kDisabledDark : _kDisabledLight,
        );
  ...
  Widget _buildHeaderText(int index) {
    if (widget.steps[index].title != null || widget.steps[index].subtitle != null) {
      return Column(
